...
def findSpeeches(raw_speeches, multiple_speakers, daily_soup, date, volno):
	...
	for talk in daily_soup.find_all('sp'):
		...
		try:
			...
			speaker = speaker.replace(".","").replace(":","").replace("-", " ").replace("MM ", "").replace("MM. ","").replace("M ", "").replace("de ","").replace("M. ","").replace("M, ","").replace("M- ","").replace("M; ","").replace("M* ","")
			if speaker.endswith(","):
				speaker = speaker[:-1]
			if speaker.endswith(", "):
				speaker = speaker[:-1]
...
if __name__ == '__main__':
	...
	speaker_distances = pd.DataFrame(speaker_dists, columns = ["Speaker Name", "Levenshtein Dists", "Volno", "Date"])
